"""
This script compares the basic `.encode` tokenization between Hugging Face and Mistral tokenizers across multiple datasets.
It identifies and reports mismatches in tokenization results, helping ensure consistency between the two tokenizers.
The comparison is limited to the first 10,000 characters to avoid excessively long sequences.
...
If no mismatch is detected, it only indicates that the first 10,000 characters are consistent—not necessarily the entire sample.
...
Usage:
    python3 compare_tokenizer.py --hf_model <hf_model_name> --mc_model <mc_model_name> [--num_samples <num_samples>] [--hf_token <hf_token>] [-save-results]
...
Examples:
    ...
    python3 compare_tokenizer.py --hf_model mistralai/Mistral-Nemo-Instruct-2407 --mc_model mistralai/Mistral-Small-3.1-24b-Instruct-2503 --num_samples 1000
"""
...
def compare_tokenizers(
    ...
    content = content[:10_000]  # Limit to 10000 characters to avoid long sequences
...
def process_dataset(
    ...
    **kwargs,
    ...
    for i, example in tqdm(enumerate(dataset), desc=f"Processing {dataset_name} ({split})", total=num_samples):
        ...
        mismatch = compare_tokenizers(hf_tokenizer, mc_tokenizer, content)
        if mismatch:
...
def test_tokenizer(
    ...
    web_mismatches = process_dataset(
        "HuggingFaceFW/fineweb", "train", hf_tokenizer, mc_tokenizer, n_per_set
    )
    ...
    pdf_mismatches = process_dataset(
        "HuggingFaceFW/finepdfs", "train", hf_tokenizer, mc_tokenizer, n_per_set
    )
    ...
    for split in splits:
        ...
        split_mismatches = process_dataset(
            "manu/project_gutenberg", split, hf_tokenizer, mc_tokenizer, n_per_split
        )
    ...
    if collected < n_per_set:
        ...
        en_mismatches = process_dataset(
            "manu/project_gutenberg", "en", hf_tokenizer, mc_tokenizer, remaining
        )
...
if __name__ == "__main__":
    ...
    results = test_tokenizer(args.hf_model, args.mc_model if args.mc_model else args.hf_model, args.n, args.hf_token)
