...
It identifies and reports mismatches in tokenization results, helping ensure consistency between the two tokenizers.
The comparison is limited to a user-specified number of characters to avoid excessively long sequences.
The mismatch rate is calculated as the percentage of samples where tokenization results differ at any point.
Results include the exact token where the mismatch starts, as well as the three tokens immediately before and after the mismatch, totaling 7 tokens.
If no mismatch is detected, it only indicates that the first `max_chars` characters are consistent—not necessarily the entire sample.
Some datasets may show a high mismatch rate due to frequent occurrences of the same problematic string, hence we recommend taking a look at the raw results.
Usage:
    python3 compare_tokenizer.py --hf_model <hf_model_name> --mc_model <mc_model_name> [--num_samples <num_samples>] [--max_chars <max_chars>] [--hf_token <hf_token>] [--save_results]
Arguments:
    --hf_model: Model name or path for the Hugging Face tokenizer.
    --mc_model: Model name or path for the Mistral-Common tokenizer loaded from Hugging Face, if not provided, will default to hf_model.
    --num_samples: Maximum number of samples to test (default: 3000).
    --max_chars: Maximum number of characters to tokenize per sample (default: 10_000).
    --hf_token: Hugging Face token for private models (optional).
    --save_results: If provided, saves the raw mismatch results to a JSON file (default: tokenizer_mismatches_data.json).
Examples:
    python3 compare_tokenizer.py --hf_model mistralai/Mistral-Small-3.1-24b-Instruct-2503 --mc_model mistralai/Mistral-Small-3.1-24b-Instruct-2503 --save_results
    python3 compare_tokenizer.py --hf_model mistralai/Mistral-Nemo-Instruct-2407 --mc_model mistralai/Mistral-Small-3.1-24b-Instruct-2503 --num_samples 1000 --max_chars 5000
"""
import argparse
...
def compare_tokenizers(
    ...
    if hf_tokens != mc_tokens:
        ...
        mc_str = [
            mc_tokenizer.decode([t], special_token_policy=SpecialTokenPolicy.KEEP)
            for t in mc_tokens[start:end]
        ]
        return {
...
def process_dataset(
    ...
    **kwargs: Any,
    ...
    for i, example in tqdm(enumerate(dataset), desc=f"Processing {dataset_name} ({split})", total=num_samples):
        if i >= num_samples:
            break
...
def test_tokenizer(
    ...
    web_mismatches = process_dataset(
        "HuggingFaceFW/fineweb", "train", hf_tokenizer, mc_tokenizer, n_per_set, max_chars
    )
    ...
    pdf_mismatches = process_dataset(
        "HuggingFaceFW/finepdfs", "train", hf_tokenizer, mc_tokenizer, n_per_set, max_chars
    )
    ...
    for split in splits:
        ...
        split_mismatches = process_dataset(
            "manu/project_gutenberg", split, hf_tokenizer, mc_tokenizer, n_per_split, max_chars
        )
    ...
    if collected < n_per_set:
        ...
        en_mismatches = process_dataset(
            "manu/project_gutenberg", "en", hf_tokenizer, mc_tokenizer, remaining, max_chars
        )
    ...
    # Total Mismatch
    print("\n=== SUMMARY ===")
    print(f"Total Mismatch Rate: {(len(web_mismatches) + len(pdf_mismatches) + len(multi_mismatches)) / num_samples * 100:.2f}% ( {len(web_mismatches) + len(pdf_mismatches) + len(multi_mismatches)} / {num_samples} )")
    print(f" - HuggingFaceFW/fineweb: {len(web_mismatches) / n_per_set * 100:.2f}% ( {len(web_mismatches)} / {n_per_set} )")
    print(f" - HuggingFaceFW/finepdfs: {len(pdf_mismatches) / n_per_set * 100:.2f}% ( {len(pdf_mismatches)} / {n_per_set} )")
    print(f" - manu/project_gutenberg: {len(multi_mismatches) / n_per_set * 100:.2f}% ( {len(multi_mismatches)} / {n_per_set} )")
    return {
...
def save_results(results: dict[str, list[dict[str, Any]]], filename: str = "tokenizer_mismatches_data.json") -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mc_model", type=str, default=None, help="Model name or path for the Mistral-Common tokenizer.")
    parser.add_argument("--n", type=int, default=3000, help="Maximum number of samples to test")
    parser.add_argument("--max_chars", type=int, default=10_000, help="Maximum number of characters to tokenize per sample")
    ...
    results = test_tokenizer(args.hf_model, args.mc_model if args.mc_model else args.hf_model, args.n, args.max_chars, args.hf_token)
