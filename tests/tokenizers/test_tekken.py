...
def merge_tekkenizer() -> Tekkenizer:
    r"""Tekkenizer with a single-character split pattern, used for byte-level roundtrip tests.

    ``pattern="."`` forces every chunk fed to BPE to be exactly one character, so no
    merge into a compound token can ever occur: encoding ``"My very beautiful string"``
    yields 24 tokens for its 24 characters. The fixture exists to exercise pure
    byte-level encode/decode roundtripping against the same default pattern used by
    `build_tekkenizer_from_file`, not to exercise BPE merging.
...
class TestTekkenizer:
    ...
    def test_from_file_roundtrip_encode(self, tmp_path: Path, merge_tekkenizer: Tekkenizer) -> None:
        loaded = build_tekkenizer_from_file(tmp_path, version="v3", special_tokens=None)
        text = "My very beautiful string"
        assert loaded.encode(text, bos=False, eos=False) == merge_tekkenizer.encode(text, bos=False, eos=False)
