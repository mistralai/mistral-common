...
def build_tekkenizer(
    ...
    *,
    ...
    `quick_vocab` and the special tokens from `get_special_tokens`. Chunking always
    uses the production `TEKKEN_PATTERN`; it is not overridable, because a pattern
    without `DOTALL` silently discards newlines.
...
def write_tekkenizer_model(
    ...
    *,
    ...
    keys for tests that exercise those `from_file` code paths. The config block always
    carries `TEKKEN_PATTERN`, the production tekken chunking pattern, so tokenizers
    loaded from the written file chunk text the same way a shipped tokenizer does.
