...
def _decode_tool_calls_v2_up_to_v7(tool_call_tokens: list[int], tokenizer: Tokenizer) -> List[ToolCall]:
    ...
    if tokenizer.version == TokenizerVersion.v2:
        return [ToolCall(function=FunctionCall.model_validate(tool_call)) for tool_call in tool_calls_decoded_list]
    return [
        ToolCall(
            id=tool_call["id"],
...
def _decode_tool_call_v11_with_call_id(tool_call_tokens: list[int], tokenizer: Tokenizer) -> ToolCall:
    ...
    try:
        tool_call = ToolCall(
            id=tokenizer.decode(call_id) if call_id is not None else "null",
...
def _decode_tool_call_v11_up_to_v13(tool_call_tokens: list[int], tokenizer: Tokenizer) -> ToolCall:
...
def decode_tool_calls(tool_call_tokens: Sequence[list[int]], tokenizer: Tokenizer) -> List[ToolCall]:
    ...
    for tool_call in tool_call_tokens:
        if tokenizer.version == TokenizerVersion.v1:
            ...
            tools_calls.append(_decode_tool_call_v11_up_to_v13(tool_call, tokenizer))
