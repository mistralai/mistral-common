...
- Test files contain only tests: shared objects and builders come from `tests/utils/`
  (e.g. `tests/utils/tokenizers.py`), tokenizers themselves come from `conftest.py` fixtures.
  `tests/tokenizers/` mirrors the `src/mistral_common/tokens/tokenizers/` package; `tests/utils_tests/`
  holds self-tests for the `tests/utils/` harness.
